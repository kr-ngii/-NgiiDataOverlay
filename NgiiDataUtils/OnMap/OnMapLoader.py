...
def importPdfVector(pdf, layerInfoList, affineTransform, crsId, mapNo, bbox):
    ...
    onmapGroup = root.addGroup(u"온맵")


    # Create QGIS Layer
    for layerInfo in layerInfoList:
        # print(layerInfo[u"name"])
        ...
        if layerInfo["pointCount"] > 0:
            ...
            onmapGroup.insertChildNode(1,QgsLayerTreeLayer(vPointLayer))
        if layerInfo["lineCount"] > 0:
            ...
            onmapGroup.insertChildNode(1, QgsLayerTreeLayer(vLineLayer))
        if layerInfo["polygonCount"] > 0:
            ...
            onmapGroup.insertChildNode(1, QgsLayerTreeLayer(vPolygonLayer))
...
def load(filepath = PDF_FILE_NAME, iface=None, parent=None):
    _iface = iface
    _parent = parent
    ...
    try:
        pdf, layerInfoList, affineTransform, crsId, mapNo, bbox, imgBox = getPdfInformation(filepath)
    except TypeError:
    ...
    imgFile = importPdfRaster(filepath, crsId, affineTransform, imgBox)
